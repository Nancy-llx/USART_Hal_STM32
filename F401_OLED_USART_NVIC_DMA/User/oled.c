...
void OLED_Init(I2C_HandleTypeDef *i2c_handle)
{
	...
	for (i = 0; i < 23; i++)
	{
		HAL_I2C_Mem_Write(I2C_HANDLE, addr_i2c, 0x00, I2C_MEMADD_SIZE_8BIT, &CMD_Data[i], 1, 0x100);
	}
	HAL_Delay(10);
	OLED_Clear();
}
...
{
	HAL_I2C_Mem_Write(I2C_HANDLE, addr_i2c, 0x00, I2C_MEMADD_SIZE_8BIT, &cmd, 1, 0x100);
}

void OLED_WR_DATA(uint8_t data)
{
	HAL_I2C_Mem_Write(I2C_HANDLE, addr_i2c, 0x40, I2C_MEMADD_SIZE_8BIT, &data, 1, 0x100);
}

void OLED_WR_DATA_LINE(uint8_t *data)
{
	HAL_I2C_Mem_Write(I2C_HANDLE, addr_i2c, 0x40, I2C_MEMADD_SIZE_8BIT, data, 128, 0x100);
}
...
/**
 * @function: void OLED_Showdecimal(uint8_t x,uint8_t y,double num,uint8_t z_len,uint8_t f_len,uint8_t size2, uint8_t Color_Turn)
 * @description: 显示正负浮点数
 * @param {uint8_t} x待显示的数字起始横坐标,x:0~126
 * @param {uint8_t} y待显示的数字起始纵坐标, y:0~7，若选择字体大小为16，则两行数字之间需要间隔2，若选择字体大小为12，间隔1
 * @param {double} num:输入的浮点型数据
...
void OLED_HorizontalShift_stop()

{
	OLED_WR_CMD(0x2e); // 停止滚动
	OLED_WR_CMD(0);	   // 设置滚动方向
	OLED_WR_CMD(0x00); // 虚拟字节设置，默认为0x00
	OLED_WR_CMD(0x00); // 设置开始页地址
	OLED_WR_CMD(0x07); // 设置每个滚动步骤之间的时间间隔的帧频
					   //   0x00-5帧， 0x01-64帧， 0x02-128帧， 0x03-256帧， 0x04-3帧， 0x05-4帧， 0x06-25帧， 0x07-2帧，
	...
	OLED_WR_CMD(0x00); // 虚拟字节设置，默认为0x00
	OLED_WR_CMD(0xff); // 虚拟字节设置，默认为0xff
	OLED_WR_CMD(0x2E); // 开启滚动-0x2f，禁用滚动-0x2e，禁用需要重写数据
}
